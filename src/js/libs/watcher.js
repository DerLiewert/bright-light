...
export class ScrollWatcher {
  constructor(items, props) {
    ...
    this.items = items;
    // !document.documentElement.classList.contains('watcher') ? this.scrollWatcherRun(items) : null;
      ...
      let uniqParams = uniqArray(
        Array.from(items).map(function (item) {
          ...
          if (
            ...
            (!item.dataset.watchThreshold || item.hasAttribute('data-watch-threshold-change'))
          ) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.addEventListener(
    'resize',
    debounce(watcherNavigator.scrollWatcherUpdate.bind(watcherNavigator), 250),
  );
