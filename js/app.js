 ...
 */
    ...
    (() => {
        ...
        class ScrollWatcher {
            ...
            scrollWatcherUpdate(items = null) {
                this.scrollWatcherOffAll();
                if (items) this.items = items;
                this.scrollWatcherRun();
            }
            scrollWatcherRun() {
                document.documentElement.classList.add("watcher");
                this.scrollWatcherConstructor(this.items);
            }
            scrollWatcherConstructor(items) {
                if (items.length) {
                    this.scrollWatcherLogging(`Прокинувся, стежу за об'єктами (${items.length})...`);
                    let uniqParams = uniqArray(Array.from(items).map((function(item) {
                        if (item.dataset.watch === "navigator" && (!item.dataset.watchThreshold || item.hasAttribute("data-watch-threshold-change"))) {
                            ...
                            item.setAttribute("data-watch-threshold-change", "");
                        }
            ...
            scrollWatcherInit(items, configWatcher) {
                ...
                this.observers.set(observer, items);
            }
            ...
            scrollWatcherOffAll() {
                this.observers.forEach(((items, observer) => {
                    items.forEach((item => {
                        this.scrollWatcherOff(item, observer);
                    }));
        ...
        document.addEventListener("DOMContentLoaded", (() => {
            const watcherNavigator = new ScrollWatcher(document.querySelectorAll('[data-watch="navigator"]'));
            flsModules.watcher = watcherNavigator;
            window.addEventListener("resize", debounce(watcherNavigator.scrollWatcherUpdate.bind(watcherNavigator), 250));
        ...
        function headerScroll() {
            ...
            let timer;
            document.addEventListener("windowScroll", onScroll);
            function onScroll(e) {
...
***************************************************************************** */
        ...
        players.forEach((player => {
            ...
            const trackItems = player.dataset.tracksList ? document.querySelectorAll(`${player.dataset.tracksList} [data-src]`) : null;
            let audio = null;
            let currentTime = 0;
            let isMoving = false;
            ...
            function onKeyDown(e) {
                ...
                if (charCode === 37) currentTime -= 5; else if (charCode === 39) currentTime += 5;
                let audioDuration = getDuration();
                if (currentTime <= 0) currentTime = 0; else if (currentTime > audioDuration) currentTime = audioDuration;
                audio.currentTime = currentTime;
            }
            function newCurrentTime(clientX, shiftX = 0) {
                ...
                if (newLeft <= 0) currentTime = 0; else if (newLeft >= progressBar.offsetWidth + thumb.offsetWidth) currentTime = duration; else currentTime = newLeft * duration / progressBar.offsetWidth;
            ...
            function endedMusic() {
                ...
                if (isMoving) return;
                let nextTrack = document.querySelector(`${player.dataset.tracksList} ._active`).nextElementSibling;
                pauseMusic();
                if (nextTrack) {
            ...
            function getDuration() {
                let audioDuration = 0;
                if (audio?.duration) audioDuration = audio.duration; else audioDuration = tracks.find((obj => obj.url === audio.getAttribute("src")))?.duration;
